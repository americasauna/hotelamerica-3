	...
	<div align="center" class="hgb2">
		...
		<form name="listaRLocal" id="listaRLocal" method="post"
			...
			<table class="patient">
				...
				%>
				<tr>
					<td>Usuario <%=i++%> <input class="hgb" type="checkbox"
						name="elegidos" value="<%=a.getId()%>">
					</td>
					<td><%=a.getName()%></td>
					<td><%=a.getDni()%></td>
					<td><%=a.getSexo()%></td>
			...
			<br> <br>
			<%if(empleado.size()!=0){ %>
			<input type="submit" value="bloquear" class="hgb" name="op">
			<input type="submit" value="activar" class="hgb" name="op">
			<%}else{%>
			<% request.setAttribute("mensajito","No hay usuarios registrados");%>
			<a href="/adm/empleado?=0">Registre un empleado primero :O</a>
			<%}%>
		</form>
	</div>
