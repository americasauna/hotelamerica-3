...
public class ProyectoSaunaServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		if(eva.equals("SPA")){
			List<Spa> spa = (List<Spa>)pm.newQuery(Spa.class).execute();
			req.setAttribute("spa", spa);
			RequestDispatcher rd = getServletContext().getRequestDispatcher("/WEB-INF/EditarSpa.jsp");
				rd.forward(req, resp);	
		}
		
		if(eva.equals("MASAJES")){
			List<Sauna> ma = (List<Sauna>)pm.newQuery(Masajes.class).execute();
			req.setAttribute("masajes", ma);
